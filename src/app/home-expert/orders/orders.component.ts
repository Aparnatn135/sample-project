...
import { ExpertOrders } from './../../common/ordertype-list';
...
this.fetchOrders(code);
  ...
  onRowClicked(event){
    this.route.navigate(['/home-expert/order-detail'], { queryParams: { orderid:event.data.id } });
  ...
  fetchOrders(code){
    ...
    if(data.success){
      if(data.orders)
      this.agGrid.api.setRowData(data.orders);
      else if(data.data)
