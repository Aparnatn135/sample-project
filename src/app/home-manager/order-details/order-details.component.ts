...
durationInSeconds = 5;
     ...
     this.fetchStats();
       ...
       fetchBids(){
        ...
        fetchStats(){
          const payload = new HttpParams()
          .set('order_id', this.order_id);
          this.httpService.posts('/api/v1/stats/test/',payload,null).subscribe(data=>{
          if(data.success){
            this.route.navigate(['/home-manager/dashboard']);
          }
          });
        }
        onRowClicked(e){
  ...
  approveOrder(){
    ...
    this.httpService.posts('/api/v1/approve_order/',payload,null).subscribe(data=>{
      if(data && data.success){
        this.router.navigate(['/home-manager/orders']);
        ...
        .set('updated_progress', this.form.value.work_percent)
         ...
         .set('attachments', attachmentId.toString());
               this.httpService.posts('/api/v1/update_progress/',payload,null).subscribe(data=>{
                      if(data && data.success){        
                       this.router.navigate(['/home-expert/assignments']);
                   }       
                  });
